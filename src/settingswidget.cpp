...
SettingsWidget::SettingsWidget(QWidget *parent)
  ...
  ui->infoLabel->setText(
      ui->infoLabel->text().arg(QApplication::applicationName()));
  ...
  ui->autoFinishDurationSpinBox->setValue(
      settings.value("autoFinishDuration", 10).toInt());
  ...
  connect(ui->themeCombo, &QComboBox::currentTextChanged, styler,
          &QtStyler::setStyle);
  connect(styler, &QtStyler::styleChanged, ui->themeCombo,
...
void SettingsWidget::on_saveKeys_clicked() {
  ...
  // load
  QByteArray base64EncodedKey =
      settings.value("yek", key.toUtf8().toBase64()).toByteArray();
  ui->keyLineEdit->setText(QString(QByteArray::fromBase64(base64EncodedKey)));
...
void SettingsWidget::on_autoFinishDurationSpinBox_valueChanged(int arg1) {
  ui->autoFinishDurationSpinBox->setSuffix(
      QString(arg1 < 2 ? " second" : " seconds"));
