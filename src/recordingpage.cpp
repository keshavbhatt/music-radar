...
RecordingPage::RecordingPage(QWidget *parent)
    : QWidget(parent), ui(new Ui::RecordingPage) {
  ui->setupUi(this);
...
void RecordingPage::startRecording(QString selectedDevice) {
  ...
  connect(recordingProcess, &QProcess::readyRead, [=]() {
    ...
    if (QString(bytes).contains("Time", Qt::CaseInsensitive)) {
      ui->statusLabel->setText(
          QString(bytes).split(";").first().trimmed().simplified());
      QString breaker =
          " " +
          QString::number(settings.value("autoFinishDuration", 10).toInt()) +
          ".";
          ...
          [this](int exitCode, QProcess::ExitStatus exitStatus) {
            ...
            if (info.size() < 800) {
              QMessageBox::critical(
                  this, "Error",
                  "Recorded file is empty.\nIf you installed using snapd make "
                  "sure you enable audio-record permission.");
              stopRecording();
              emit back();
            }
          });
  ...
  recordingProcess->start(
      "bash", QStringList()
                  << "-c"
                  << QString("parec -v -d %1 | lame -r -V0 - %2")
                         .arg(selectedDevice,
                              QString(recordingFileName).replace(" ", "\\ ")));
  ...
  if (!recordingProcess->waitForStarted(1000)) {
    ...
    QMessageBox::critical(
        this, "Error",
        "Failed to initialize recording process.\nPlease report to developer.");
    stopRecording();
    emit back();
  } else {
    ui->cancelButton->show();
...
void RecordingPage::clearDebug() { debug.clear(); }
  ...
  foreach (QProcess *process, this->findChildren<QProcess *>()) {
    ...
    for (int i = 0; i < processIdList.count(); i++) {
      QProcess::execute("pkill", QStringList()
                                     << "-P"
                                     << QString::number(processIdList.at(i)));
...
void RecordingPage::on_cancelButton_clicked() {
  ...
  if (info.size() > 800) {
    ...
    QMessageBox::critical(this, "Error",
                          "Recorded file is too small, try recording again.");
    stopRecording();
    emit back();
  }
}
...
void RecordingPage::processRecording() {
  ...
  QString t = "";
  QByteArray base64EncodedKey =
      settings.value("yek", t.toUtf8().toBase64()).toByteArray();

  QString bound = "margin";
  QByteArray data(QString("--" + bound + "\r\n").toLatin1());
  data.append("Content-Disposition: form-data; name=\"api_token\"\r\n\r\n");
  data.append(QString(QByteArray::fromBase64(base64EncodedKey)).toUtf8() + "\r\n");
  ...
  connect(networkReply, SIGNAL(uploadProgress(qint64, qint64)), this,
...
void RecordingPage::uploadFinished(bool fromHistory,
                                   QString historyItemFilePath) {
  _loader->stop();
  ...
  if (status.contains("error", Qt::CaseInsensitive)) {
    ...
    QString error_message =
        jsonObj.value("error").toObject().value("error_message").toString();
    int error_code =
        jsonObj.value("error").toObject().value("error_code").toInt();
    if (error_code == 901) {
