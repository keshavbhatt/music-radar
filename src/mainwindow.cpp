...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(recordingPageWidget, &RecordingPage::showJson, [=](QString jsonPath) {
    ...
    info->setWindowTitle(QApplication::applicationName() + " | " +
                         tr("Item info"));
  ...
  connect(recordingPageWidget, &RecordingPage::showFixKeyError,
          [=](QString error) {
            QMessageBox msgBox;
            ...
            QPushButton *settingsButton = msgBox.addButton(
                tr("Yes, correct API Token"), QMessageBox::ActionRole);
  ...
  connect(
      recordingPageWidget, &RecordingPage::showPutKeyError, [=](QString error) {
        QMessageBox msgBox;
        ...
        QPushButton *settingsButton =
            msgBox.addButton(tr("Yes, set API Token"), QMessageBox::ActionRole);
        QPushButton *abortButton = msgBox.addButton(QMessageBox::No);
        msgBox.exec();
        if (msgBox.clickedButton() == settingsButton) {
          settingsAction->trigger();
  ...
  connect(recordingPageWidget, &RecordingPage::enableItemActions,
          [=](bool enable) {
  ...
  RateApp *rateApp = new RateApp(this, "snap://msuci-radar", 5, 5, 1000 * 30);
...
void MainWindow::createActions() {
  aboutAction =
      new QAction(QIcon(":/icons/app/icon-32.png"), tr("&About"), this);
  backAction = new QAction(QIcon(":/icons/arrow-left-circle-line.png"),
                           tr("&Back"), this);
  historyAction =
      new QAction(QIcon(":/icons/time-line.png"), tr("&History"), this);
  itemDeleteAction =
      new QAction(QIcon(":/icons/delete-bin-5-line.png"), tr("Delete"), this);
  itemInfoAction =
      new QAction(QIcon(":/icons/information-line.png"), tr("Info"), this);
  settingsAction =
      new QAction(QIcon(":/icons/setting-line.png"), tr("Settings"), this);

  connect(settingsAction, &QAction::triggered, this, &MainWindow::showSettings);

  connect(itemDeleteAction, &QAction::triggered, recordingPageWidget,
          &RecordingPage::deleteSelected);

  connect(itemInfoAction, &QAction::triggered, recordingPageWidget,
          &RecordingPage::showItemInfo);
...
void MainWindow::init_settings() {
  ...
  settingsWidget->restoreGeometry(
      settings.value("settingsGeometry").toByteArray());
}
